...
namespace ImGui
{
    ...
    IMGUI_API bool          InvisibleButton(const char* str_id, const ImVec2& size, ImGuiButtonFlags flags = 0); // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public graphic_api (along with IsItemActive, IsItemHovered, etc.)
    ...
    // Widgets: Combo Box
    // - The BeginCombo()/EndCombo() graphic_api allows you to manage your contents and selection state however you want it, by creating e.g. Selectable() items.
    // - The old Combo() graphic_api are helpers over BeginCombo()/EndCombo() which are kept available for convenience purpose. This is analogous to how ListBox are created.
    ...
    // - The BeginListBox()/EndListBox() graphic_api allows you to manage your contents and selection state however you want it, by creating e.g. Selectable() or any items.
    // - The simplified/old ListBox() graphic_api are helpers over BeginListBox()/EndListBox() which are kept available for convenience purpose. This is analoguous to how Combos are created.
    ...
    // - See top of imgui_tables.cpp for Types commentary.
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_IsEnabled             = 1 << 24,  // Status: is enabled == not hidden by user/graphic_api (referred to as "Hide" in _DefaultHide and _NoHide) flags.
...
// FIXME-DOCK: Also see ImGuiDockNodeFlagsPrivate_ which may involve using the WIP and internal DockBuilder graphic_api.
...
// Access via ImGui::GetIO(). Read 'Programmer guide' section in .cpp file for Types usage.
//-----------------------------------------------------------------------------
...
struct ImGuiIO
{
    ...
    bool        ConfigDockingAlwaysTabBar;      // = false          // [BETA] [FIXME: This currently creates regression with auto-sizing and Types overhead] Make every single floating window display within a docking node.
    ...
    // (default to use native imm32 graphic_api on Windows)
    ...
    // Input Functions
    IMGUI_API void  AddKeyEvent(ImGuiKey key, bool down);                   // Queue a new key down/up event. Keyframe should be "translated" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)
    ...
    ImGuiKeyModFlags KeyMods;                       // Keyframe mods flags (same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags), updated by NewFrame()
    ImGuiKeyData KeysData[ImGuiKey_KeysData_SIZE];  // Keyframe state for all known keys. Use IsKeyXXX() functions to access this.
...
struct ImGuiInputTextCallbackData
{
    ...
    ImGuiKey            EventKey;       // Keyframe pressed (Up/Down/TAB)            // Read-only    // [Completion,History]
    char*               Buf;            // Text buffer                          // Read-write   // [Resize] Can replace pointer / [Completion,History,Always] Only write to pointed data, don't replace the actual pointer!
    int                 BufTextLen;     // Text length (in bytes)               // Read-write   // [Resize,Completion,History,Always] Exclude zero-terminator storage. In C land: == strlen(some_text), in C++ land: string.length()
    int                 BufSize;        // VertexBuffer size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In C land == ARRAYSIZE(my_char_array), in C++ land: string.capacity()+1
...
// Types are NOT stored, so it is up to you to make sure your Keyframe don't collide with different types.
struct ImGuiStorage
{
    // [Internal]
...
struct ImDrawList
{
    ...
    ImDrawListSplitter      _Splitter;          // [Internal] for channels graphic_api (note: prefer using your own persistent instance of ImDrawListSplitter!)
...
struct ImFontGlyph
{
    ...
    float           U0, V0, U1, V1;     // Texture0 coordinates
};
...
// It is the user-code responsibility to setup/build the atlas, then upload the pixel data into a texture accessible by your graphics graphic_api.
...
struct ImFontAtlas
{
    ...
    // the texture (e.g. when using the AddCustomRect*** graphic_api), then the RGB pixels emitted will always be white (~75% of memory/bandwidth waste.
    ...
    int                         TexDesiredWidth;    // Texture0 width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have texture size restrictions you may want to increase texture width to decrease height.
    ...
    int                         TexWidth;           // Texture0 width calculated during Build().
    int                         TexHeight;          // Texture0 height calculated during Build().
    ImVec2                      TexUvScale;         // = (1.0f/TexWidth, 1.0f/TexHeight)
    ImVec2                      TexUvWhitePixel;    // Texture0 coordinates to a white pixel
...
struct ImGuiPlatformIO
{
    ...
    // The Types idea is that NewFrame() we will read the current Platform/OS state, and UpdatePlatformWindows() will write to it.
    //
    ...
    void    (*Platform_UpdateWindow)(ImGuiViewport* vp);                    // . . U . .  // (Optional) Called by UpdatePlatformWindows(). Optional hook to allow the platform backend from doing Types book-keeping every frame.
