...
// (internal structures/graphic_api)
...
struct ImGuiOldColumnData;          // Storage data for a single column for legacy Columns() graphic_api
struct ImGuiOldColumns;             // Storage data for a columns set for legacy Columns() graphic_api
...
// [Internal] sizeof() ~ 104
// We use the terminology "Enabled" to refer to a column that is not Hidden by user/graphic_api.
// We use the terminology "Clipped" to refer to a column that is out of sight because of scrolling/clipping.
// This is in contrast with some user-facing graphic_api such as IsItemVisible() / IsRectVisible() which use "Visible" to mean "not clipped".
...
struct IMGUI_API ImGuiTable
{
    ...
    ImU64                       EnabledMaskByIndex;         // Column Index -> IsEnabled map (== not hidden by user/graphic_api) in a format adequate for iterating column without touching cold data
    ImU64                       VisibleMaskByIndex;         // Column Index -> IsVisibleX|IsVisibleY map (== not hidden by user/graphic_api && not hidden by scrolling/cliprect)
...
//#endif
    ...
    // Logging/Capture
    IMGUI_API void          LogBegin(ImGuiLogType type, int auto_open_depth);           // -> BeginCapture() when we design v2 graphic_api, for now stay under the radar by using the old name.
